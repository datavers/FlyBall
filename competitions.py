...
def pandas_processor():
        ...
        for sheet_name in sheet_names:
            ...
            for index, row in df.iloc[2:].iterrows():
                if row.isnull().all():
                    ...
                    dogs = [(row['name' + str(i)], row['start' + str(i)], row['time' + str(i)]) for i in range(1, 5)]
    ...
    csv_file_path = '/Users/radnar/Documents/Development/Projects/Data36_Flyball/Sources/elvart.csv'

    # A DataFrame adatok kiírása CSV fájlba
    result_headers = ['city', 'race_name', 'date', 'when', 'Division', 'who', 'total time', 'W/L/T', 'Hurdles', 'name', 'start', 'time', 'error', 'anomaly', 'w_pair']
